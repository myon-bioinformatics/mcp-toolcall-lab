...
their P0 pair, plus the P1 OpenAI-compatible adapter:
...
- **`openai_compat`** (`jev_openai_compat.OpenAICompatBackend`) — sends a
  constrained `/v1/chat/completions` request to an injected OpenAI-compatible
  transport and validates the completion against the same real TypeSafe
  `noul`/`choice`/`score` wire. Its `prob_source` is `self_reported` because
  confidence/probabilities are generated by the generic LLM, not native Jev logits.
...
backends shipped so far use `fixture`, `typesafe_mock`, and `self_reported`.
...
`openai_compat`, P2 Open Jev / LocalJev, P3 TypeSafe live), P0 and P1 are
now represented. The P1 adapter targets the existing digest-pinned llama.cpp
`cpu-llm:8080/v1/chat/completions` path, while real GGUF execution remains
opt-in and is not part of default `pytest`. Not included here, and
intentionally deferred to later, independently mergeable PRs:
...
`fixtures/jev_backend/sample_answers.json`, `tests/test_jev_backend.py`, and
`tests/test_jev_openai_compat.py` run under plain `pytest -q` — no network, no `TYPESAFE_API_KEY`, no opt-in
