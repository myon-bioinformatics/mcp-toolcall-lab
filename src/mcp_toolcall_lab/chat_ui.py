...
def login(page: Any, frontend: ChatFrontend, base_url: str, email: str, password: str) -> bool:
...
def send_in_browser(
    *,
    ...
    with sync_playwright() as playwright:
        ...
        try:
            logged_in = login(page, frontend, base_url, email, password)
            if logged_in:
...
def main(argv: list[str] | None = None) -> int:
    ...
    record = write_observation(
        Path(args.observe),
        observation={**classified, **flags, "prompt": args.prompt, "url": base},
